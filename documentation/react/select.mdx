---
title: Tailwind CSS Select for React - ZigOps Material
description: Customise your web projects with our beautiful select component for Tailwind CSS and React using Material Design guidelines.
navigation:
  [
    "select",
    "select-variants",
    "select-sizes",
    "select-colors",
    "select-validations",
    "custom-select-animation",
    "disabled-select",
    "countries-select",
    "props",
    "theme",
  ]
related:
  [
    {
      title: "Props",
      route: "props/select",
      description: "Learn more about props definition and types of select components.",
    },
    {
      title: "Theme",
      route: "theme/select",
      description: "Learn how to customize the theme and styles for select components.",
    },
  ]
github: select
prev: rating-bar
next: slider
---

# Tailwind CSS Select - React

Use our Tailwind CSS <Code>Select</Code> component to collect user provided information from a list of options.

A <Code>Select</Code> component is a dropdown menu for displaying choices. Use the radio component when there are fewer total options (less than 5).

See below our <Code>Select</Code> component example that you can use in your Tailwind CSS and React project. The example comes in different colors and styles, so you can adapt it easily to your needs.

<br />

<CodePreview id="select" link="select#select" component={
  <div className="w-72">
    <Select label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="w-72">
      <Select label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Select Variants

The <Code>Select</Code> component comes with 3 different variants that you can change it using the <Code>variant</Code> prop.

<CodePreview id="select-variants" component={
  <div className="flex flex-col w-72 gap-6">
    <Select variant="static" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
    <Select variant="standard" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
    <Select variant="outlined" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="flex flex-col w-72 gap-6">
      <Select variant="static" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
      <Select variant="standard" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
      <Select variant="outlined" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Select Sizes

The <Code>Select</Code> component comes with 2 different sizes that you can change it using the <Code>size</Code> prop.

<CodePreview id="select-sizes" component={
  <div className="flex flex-col w-72 gap-6">
    <Select size="md" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
    <Select size="lg" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="flex flex-col w-72 gap-6">
      <Select size="md" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
      <Select size="lg" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Select Colors

The <Code>Select</Code> component comes with 19 different colors that you can change it using the <Code>color</Code> prop.

<CodePreview id="select-colors" component={
  <div className="flex w-72 flex-col gap-6">
    <Select color="blue" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
    <Select color="purple" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
    <Select color="teal" label="Select Version">
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="flex w-72 flex-col gap-6">
      <Select color="blue" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
      <Select color="purple" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
      <Select color="teal" label="Select Version">
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Select Validations

The <Code>Select</Code> component comes with error and success states for performing validations.

<CodePreview id="select-validations" component={
  <div className="flex flex-col w-72 gap-6">
    <Select label="Select Version" error>
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
    <Select label="Select Version" success>
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="flex flex-col w-72 gap-6">
      <Select label="Select Version" error>
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
      <Select label="Select Version" success>
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Custom Select Animation

You can modify the open/close state animation for <Code>Select</Code> component using the <Code>animate</Code> prop.

<CodePreview id="custom-select-animation" component={
  <div className="w-72">
    <Select
      label="Select Version"
      animate={{
        mount: { y: 0 },
        unmount: { y: 25 },
      }}
    >
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="w-72">
      <Select
        label="Select Version"
        animate={{
          mount: { y: 0 },
          unmount: { y: 25 },
        }}
      >
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Disabled Select

A <Code>Select</Code> could be disabled as well, it will help you to prevent user interactions like click or focus over the <Code>Select</Code> component.

<CodePreview id="disabled-select" component={
  <div className="w-72">
    <Select label="Select Version" disabled>
      <Option>ZigOps Material HTML</Option>
      <Option>ZigOps Material React</Option>
      <Option>ZigOps Material Vue</Option>
      <Option>ZigOps Material Angular</Option>
      <Option>ZigOps Material Svelte</Option>
    </Select>
  </div>
}>
```jsx
import { Select, Option } from "@zigops-material/react";

export default function Example() {
  return (
    <div className="w-72">
      <Select label="Select Version" disabled>
        <Option>ZigOps Material HTML</Option>
        <Option>ZigOps Material React</Option>
        <Option>ZigOps Material Vue</Option>
        <Option>ZigOps Material Angular</Option>
        <Option>ZigOps Material Svelte</Option>
      </Select>
    </div>
  );
}
```
</CodePreview>

---

## Countries Select

See the example below to see how you can create a <Code>Select</Code> component with a list of countries.

<CodePreview id="countries-select" component={
  <CountriesSelect />
}>
```jsx
import React from "react";
import { useCountries } from "use-react-countries";
import { Select, Option } from "@zigops-material/react";

export default function CountriesSelect() {
  const { countries } = useCountries();

return (

<div className="w-72">
  <Select
    size="lg"
    label="Select Country"
    selected={(element) =>
      element &&
      React.cloneElement(element, {
        className: "flex items-center px-0 gap-2 pointer-events-none",
      })
    }
  >
    {countries.map(({ name, flags }) => (
      <Option key={name} value={name} className="flex items-center gap-2">
        <img
          src={flags.svg}
          alt={name}
          className="h-5 w-5 rounded-full object-cover"
        />
        {name}
      </Option>
    ))}
  </Select>
</div>
); }

```
</CodePreview>

<span id="props" className="scroll-mt-48" />
<span id="theme" className="scroll-mt-48" />
```
